...
function v.getAllNodes()
    if v.__allnodes then
        return v.__allnodes
    end
...
function v.forAllNodes(f, param, filter, fparam)
    local nodes = v.getAllNodes()
    if not filter then
    ...
    elseif type(filter) == "string" and rawget(_G, "node_getLabel") then
        for _, n in pairs(nodes) do
            if node_getLabel(n) == filter:lower() then
                if f(n, param) == true then
                    return true
                end
            end
